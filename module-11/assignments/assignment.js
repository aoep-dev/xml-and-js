...
const getGenres = async (token) => {
	const result = await fetch("https://api.spotify.com/v1/browse/categories?locale=DO", {
		method: "GET",
		headers: {
...
const renderGenres = async (filterTerm) => {
	...
	const no_tracks = document.getElementById("hide_tracks").checked;
	const no_playlist = document.getElementById("hide_playlists").checked;
						...
						tracksList = playlistTracks
							.map(({ track }) => {
								const artists = track.artists
									.map(({ name }) => name)
									.join(`, `);
								return no_tracks ? `` : `<li>${track.name} - ${artists}</li>`;
